...
struct ImageSliderViewVortexRGBPOK3R: View {
    
    // 1
    private let images = ["VortexRGBPOK3R", "VortexRGBPOK3R", "VortexRGBPOK3R"]
    ...
    var body: some View {
        ...
        ScrollView{
            ...
            VStack{
                ...
                VStack{
                    ImageSliderViewVortexRGBPOK3R()
                    .frame(height: 210)
                    .padding(.top)
                    
                
                Text("Клавиатура проводная Vortex RGB POK3R")
                    .font(.title3)
                    .fontWeight(.semibold)
            ...
            VStack {
                ...
                Divider()
                NavigationLink(destination: VortexRGBPOK3RDescription(), label: {
                    Text("Description")
                    .foregroundColor(Color.primary)
                ...
                Divider()
                NavigationLink(destination: VortexRGBPOK3RSpecs(), label: {
...
struct VortexRGBPOK3RDescription: View {
    
    var body: some View {

        ScrollView{
        Text("""
            Если вы когда-то слышали о Vortex, то почти наверняка это была модель POK3R. Нет, это не опечатка, это уже третья реинкарнация самой известной модели тайваньского производителя. Рецепт проверен годами и тысячами счастливых обладателей – берём 60% форм-фактор, добавляем переключатели Cherry MX по вашему вкусу, сверху украшаем клавишами из PBT пластика, всё это подаём в корпусе из литого алюминия. Печатайте на здоровье!
            """)
            .font(.title3)
            .padding()
            Spacer()
...
struct VortexRGBPOK3RSpecs: View {
...
struct VortexRGBPOK3R_Previews: PreviewProvider {
